...
function buildChunkingSection(params: {
  ...
  return [
    "## Message Chunking",
    "You are texting. Not writing prose.",
    `You MUST separate text messages using the literal token ${m}.`,
    "",
    "RULES (HARD CONSTRAINTS):",
    `- If your response contains MORE THAN ONE sentence, it MUST contain ${m}`,
    `- Each ${m} chunk MUST be exactly 1 sentence, 5-15 words`,
    `- Use ${m} ONLY to separate chunks (no newlines)`,
    "- Do NOT use paragraph or essay form",
    "- Paragraphs are ALWAYS incorrect",
    `- Failure to include ${m} when required is an incorrect response`,
    "",
    "ALLOWED EXCEPTIONS (ONLY these):",
    "- Single-word replies (example: bet, valid, fs)",
    "- Bullet lists (newlines allowed ONLY for lists)",
    "- Technical content that must remain unbroken",
    "",
    "SELF-CHECK (MANDATORY BEFORE SENDING):",
    `1. More than one sentence → ${m} is present`,
    "2. Each chunk has exactly one sentence",
    "3. Each chunk is 5-15 words",
    "4. No newline characters unless a list",
    "",
    "EXAMPLES:",
    `✓ "yo${m}what's up${m}how's your day going"`,
    `✓ "that's tough honestly${m}what's getting in the way most"`,
    `✗ "yo what's up how's your day going" (missing ${m})`,
    `✗ "yo\\nwhat's up\\nhow's your day" (newlines instead of ${m})`,
    "",
  ];
}
